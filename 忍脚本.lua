...
function scripts()							
local win = ui:new(""..REN["Name"]..""..REN["版本"].."")
--
local UITab1 = win:Tab("『信息』",'6035145364')--1
...
local a1 = win:Tab("刀刃球",'6035145364')--11
local JS1 = win:Tab("极速传奇",'6035145364')--12
local RZCQ1 = win:Tab("忍者传奇",'6035145364')--13
local li1 = win:Tab("力量传奇",'6035145364')--14
local FM1 = win:Tab("伐木大亨2",'6035145364')--15
local hjfsmnq = win:Tab("火箭发射模拟器",'6035145364')--16
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    bin:Label("v2.1 极速传奇自动重生/刷等")
    bin:Label("v2.0 奎尔湖 刀刃球")
    bin:Label("v1.0.1 进击的僵尸")
    bin:Label("v1.0↓ 俄亥俄州 监狱人生 伐木大亨2 力量传奇")
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
local function lumbsmasher_legitpaint(wood_class, blueprint, tpback)
    ...
    while Sawed.Parent ~= nil do
        ...
        if iterations > 50 then
            ...
            notify(''..REN["Name"], "尝试太多次蓝图填充木头了", 4)
            -- end
        end
        iterations = iterations + 1
...
local function getBestAxe(treeClass)
    ...
    if #tools == 0 then
        return notify(''..REN["Name"], "你需要斧头", 4)
    end
    local toolStats = {}
    local tool
    ...
    if not tool and treeClass == "LoneCave" then
        return notify(''..REN["Name"], "你需要末日斧头", 4)
    end
...
local function bringTree(treeClass)
    ...
    if not tree then
        return notify(''..REN["Name"], "没有找到树", 3)
    end
...
local function donate(plr, amount)
    ...
    notify(''..REN["Name"], '转钱' .. tostring(amount) .. ' 给 ' .. tostring(plr), 4);
    scr2.exitNotice();
end
...
function CanClientLoad()
    if not game:GetService "ReplicatedStorage".LoadSaveRequests.ClientMayLoad:InvokeServer(lp) then
        notify(''..REN["Name"], "等待加载时间,请耐心的等待", 4)
        repeat
            game:GetService "RunService".Stepped:wait()
...
function autobuyv2(o)
    ...
    if item == nil then
        notify(''..REN["Name"], "没有找到商品或者没有刷新，请你等待", 4)
        repeat
            task.wait()
            item = finditem(o)
        until item ~= nil
...
Section:Button("获取当前死亡后加载时间", function()
    bailib:Notification(''..REN["Name"], " 时间为" .. bai.loaddupeaxewaittime .. "秒", "好")
end)
    ...
    if CheckSlot ~= false then
        if CheckIfSlotAvailable(CheckSlot) == true then
            ...
            if LoadSlot == false then
                notify(''..REN["Name"], "有冷却现在不能加载!", 1)
            end
            if LoadSlot == true then
                notify(''..REN["Name"], "已加载!", 2)
                CurrentSlot = CheckSlot
            end
        else
            notify(''..REN["Name"], "貌似不工作了", 2)
        end
    else
        notify(''..REN["Name"], "请你填数字☹️", 2)
    end
...
Sectionzhenglimt:Button("开始整理", function()
    if bai.zlmt == nil then
        return notify(''..REN["Name"], "你没有选择木头", 4)
    end
...
Sectionchuanson:Button("获得工具", function()
    if bai.itemset == nil then
        return notify(''..REN["Name"], "请你放传送点", 4)
    end
...
Sectionhuanjin:Button("船离开时间", function()
    ...
    if game.Workspace.Ferry.TimeToDeparture.Value == 0 then
        notify(''..REN["Name"], "船已离开", 4)
    else
        notify(''..REN["Name"], "距离船离开时间有" .. game.Workspace.Ferry.TimeToDeparture.Value .. "秒", 4)
    end
end)
Sectionhuanjin:Button("桥上升时间", function()

    if game.Workspace.Bridge.DownTime.Value < 0 then
        notify(''..REN["Name"], "桥已上升", 4)
    else
        notify(''..REN["Name"], "距离桥上升的时间有" .. game.Workspace.Bridge.DownTime.Value .. "秒", 4)
    end
end)
Sectionhuanjin:Button("查看游戏时间", function()
    notify(''..REN["Name"], "游戏时间为" .. tostring(game.Lighting.TimeOfDay):sub(1, -2):sub(1, -2):sub(1, -2), 4)
end)
...
Sectionmogui:Toggle("查看玩家基地", 'Toggleflag', false, function(state)
    ...
    if state then
        if see == nil then
            return notify(''..REN["Name"], "没有找到基地", 4)
        end
...
Tab:Button("星月交辉 V3[破解]", function()
    game.Players.LocalPlayer.Character.Name = "tangxianchu"
    ...
    --星月交辉 V3 模块加载
    use_load(SCC_CharPool[1])
    end)

	
    Tab:Button("道庭DT[破解]", function()
    game.Players.LocalPlayer.Name = "123fa98"
    game.Players.LocalPlayer.Character.Name = "123fa98"
    ...
    Tab:Button("小白脚本[破解]", function()
    game.Players.LocalPlayer.Name = "4v5n7n"
    game.Players.LocalPlayer.Character.Name = "4v5n7n"
    loadstring(game:HttpGet(('https://raw.githubusercontent.com/xiaobaijiaoben/-X-/main/%E5%B0%8F%E7%99%BD1L.lua'),true))()
    --客户版
    --小白功能已恢复
    end)
    ...
    use_load("https://shz.al/~Dashutong")
...
if game.Players.LocalPlayer.DisplayName == "renscriptnb" then
game.Players.LocalPlayer:Kick(REN["游戏名2"].."请支持正版\n")
end
